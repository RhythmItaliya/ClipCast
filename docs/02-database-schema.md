...
- **`Clip`** — one row per rendered output clip, linked to its parent
  ...
  to the owning `User`. `title` is AI-generated by Gemini alongside moment
  selection (see [06-video-processing-pipeline.md](06-video-processing-pipeline.md));
  `thumbnailS3Key` points at a JPEG frame grabbed from the rendered clip;
  `duration` is seconds. All three are nullable — clips rendered before this
  feature existed have `null` here, and the UI falls back to a
  filename-derived title and a placeholder thumbnail for those.
...
model Clip {
    ...
    title          String?  // AI-generated (Gemini, same call as moment selection)
