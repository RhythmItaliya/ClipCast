...
| [`03-video-processing.excalidraw`](03-video-processing.excalidraw) | Inside the GPU processor Modal app: model loading, transcription, LLM moment selection (DeepSeek/Gemini/Claude), the preview-vs-full branch, and the ASD → reframe → caption render chain. |
| [`04-youtube-ingestion.excalidraw`](04-youtube-ingestion.excalidraw) | Two separate YouTube integrations: Lane A (paste a URL → proxy-rotated download → S3) and Lane B (OAuth channel-connect → token refresh → channel video list), including the daily auto-clip cron (now fully built). |
...
> **Note:** these diagrams cover the clip pipeline and billing. They predate the
> **Audio Studio** (docs/14–16, 19) and the **multi-agent production crew**
> (docs/17–18); for those, read the prose docs until the diagram set is
